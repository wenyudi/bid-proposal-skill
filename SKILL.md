...
- 先判**制胜轴**（管理制胜 / 创意制胜，见 strategy-patterns——创意标别默认 logic、signature 可以是创意 KV），再发散 ≥3 个主张角度、按短板+差异化收敛一个，被否角度各留一句理由。产出制胜一页纸（落 `_研判.md`）：客户张力 / 尖锐洞察 / 核心主张 + 十秒记忆句 / 推导链（洞察→策略→表达→执行→证明）/ **最强替代命题 + 为何不选** / 互换测试结论（换成对手是否仍成立）/ 拿分打法（地板怎么保、天花板怎么顶）/ 逐章骨架（每章：独有贡献、认领评分项、`{PER_SECTION_CHARS}`、前后交接）/ 唯一 signature 主亮点 / 叙事选择（自 `narratives.json` 按标型自动选）。参考 `references/strategy-patterns.md`。

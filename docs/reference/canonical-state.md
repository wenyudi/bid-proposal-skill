...
v3 把可修改事实集中在五份 canonical JSON。正文、compiled brief、coverage、diagnostics、customer-fit、realization 和 presentation blueprint 都是派生产物，不能反向静默改写 canonical。
...
`presentation-blueprint/v1` 从 valid realization 压缩出 core/appendix 页序、唯一 signature/sample 页、上屏文案、画面任务和素材请求。它只引用 audited brief 白名单，不进入 canonical 写权限，也不能把参考 PPT 的内容提升为本项目事实。

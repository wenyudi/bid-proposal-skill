...
`_state/` 保存 canonical 和 sections，但旧 brief/realization 的 compiled path 不跨目录续用。复制到新目录后必须复核一页纸策略批准、比较式取舍和 signature 主亮点，重新冻结/编译全部 snapshot-bound brief，复审正式章节和综述，再对新 report 运行 `validate-run`/`finalize-run`。v3.3 不使用 artifact registry。

...
| Task 1 | 标书与规则、标型先验 | 五个小组件 + candidate 一页纸 + bootstrap 索引 | 通过 bootstrap / 主 agent | 组件可读；候选池与策略假设未被过早收窄 |
...
| Task 2.5 | value-selection brief + 策略骨架；初稿后 rubric | 一页纸策略、lead/supporting/reserve、Section spine、Claim/Action/visible output | `task2.5` 的窄权限 | research-informed；不自批 Gate/策略，不扩 authority |
...
Task 2.5 可以选择、组合、编排，收敛一页纸策略 / Big Idea / narrative / Section strategy role，或要求重开 Gate，但不能：
...
Task 2.5 后先编译 `strategy-review`。人工只看张力、洞察、主张/记忆句、推导、互换测试、落地可信度、五维弱项和逐章贡献，然后一次批准；要求修改就回 Task 2.5。`-auto` 再运行一次 `apply-auto-state` 把 ready-for-review 页写成 assumed。pending/changes_requested 阻断分章，assumed 阻断 submission。
...
Task 3 writer 读取 compiled brief 与 `references/writing-patterns.md`，只写 Markdown。反模式库留给独立 critic。独立 auditor 可按 profile 每批 2–3 章，但逐章输出 semantic 文件、禁止跨章引用；工具直接验证 quote/brief/snapshot，不再依赖 writer hints 或 artifact registry。Task 3、Task 3.5 和红队都没有 canonical 写权限。
...
- `strategy_critic`：一句话复述、洞察锐度、推导闭环、名称互换、逐章贡献与落地可信度；

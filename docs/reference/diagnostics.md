...
`reading_efficiency` 不从 visible output 是否 filled 推断。`validate-run` 只会提升当前 snapshot 且 exact quote 能在当前报告定位的 strategy_critic/integrated 判断；旧报告、stale 红队或悬空引用保持 `not_evaluated`。

...
同一个 Need 对不同角色并不等价。业务负责人关注省心，财务纪检可能关注留痕，评标专家关注方法是否成立。v3 因此按有依据的 `Role × Need × Criterion` 路径检查覆盖，而不是只列一个笼统“客户需求清单”。
...
- 未知能力或新增成本进入 Gate；
- 只有 selected / publishable 才需要完整的客户路径、证据、authority 和交付边界。
...
重大能力、报价或偏好取舍进入 Gate，不由叙事策略暗中决定。CustomerDependency 也不能把我方责任转嫁给客户，必须提供延误影响、安全 fallback 和升级路径。

## DecisionJob 如何改善阅读

每个正式章节只有一个 primary DecisionJob、最多一个 secondary。它描述评委读完本章应形成的新判断，例如“相信该机制能持续产出”或“确认风险和预算可控”。
...
customer-fit 用十个维度检查客户路径、证据、价值、差异化、交付、承诺和阅读效率。它的区间用于回答“下一轮最值得修什么”，而不是“我们能得多少分”。

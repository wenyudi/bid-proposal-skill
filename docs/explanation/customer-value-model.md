...
同一个 Need 对不同角色并不等价。业务负责人关注省心，财务纪检可能关注留痕，评标专家关注方法是否成立。v3.1 用一个轻量 `DecisionPath` 直接记录有依据的 `Role × Need × Criterion`，而不是维护三套可漂移 link 或只列一个笼统“客户需求清单”。
...
Big Idea 是 selected ValueProposition 的记忆与叙事伞。它帮助评委记住方案，但不替代原子价值，也不能掩盖证据或交付短板。
...
因此模型不会在灵感刚出现时就要求它像合同承诺一样完备。它只是防止未经验证的候选在收敛过程中悄悄变成客户承诺。
...
每个 lead VP 还要在所属章给出一个小型 `visible_output` contract：它只规定评委要看到什么、必填字段和 truth boundary，不规定表格或版式。这样保留写作发散，同时防止“流程很完整，正文却没有一个可检查对象”。

## customer-fit 的作用

customer-fit 用十个维度检查客户路径、证据、价值、差异化、交付、承诺和阅读效率。它输出 withheld / fragile / credible / competitive / strong 的内部 ordinal rating 和 top gaps，不输出伪精确权重/区间，也不是“我们能得多少分”。未评价 differentiation 或 reading efficiency 时，不能称 competitive/strong。

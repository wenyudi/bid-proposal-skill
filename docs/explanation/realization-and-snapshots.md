...
Requirement addressed 和 Claim entailed 仍可能只得到一段正确但抽象的文字。Section 的轻量 `visible_outputs` 只规定：评委要检查什么、必须填哪些字段、依据来自哪里、什么事实不能扩大；`display_role` 再从中选出唯一 signature 主亮点，其余对象服务支撑和查验。

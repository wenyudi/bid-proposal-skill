...
pre-change 必须来自已经冻结的旧 release/worktree 或此前保存的仓库外 run；当前 v3.4 工作树只能产生 post-change。记录固定 commit、policy 和测试结果：
...
pre-change 阶段按正文人工判定；不要假装已有结构化成果 manifest。v3.4 post-change 可以读取 authoritative `visible_output_realizations`、signature 层级、presentation blueprint 和 report-anchored fit judgments，但仍须复核正文、结构稿和 strategy critic 的实际引用。

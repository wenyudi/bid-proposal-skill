...
复制的 canonical 和 `sections/` 可以作为恢复材料，但旧 brief/realization 的 compiled path 不构成新目录的有效证明。v3.3 不使用 artifact registry。继续写作前必须复核迁移生成的 assumed 一页纸策略，并补齐比较式取舍与 signature 主亮点：

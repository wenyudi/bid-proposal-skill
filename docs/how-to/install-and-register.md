...
---
name: proposal
description: 政企传媒技术标 v3.4 生成：拆标书硬要求，建立客户价值与真实交付边界，研究后比较策略命题，收敛一页纸主线和一个客户主亮点，以同一主线生成可兑现正文或图片 PPT 结构稿，独立审计和硬门定稿。Use when 用户要写投标方案/应标文件/政企客户提案/PPT 提案结构，提供标书要求出方案，或输入 /proposal。
---
...
2. 严格执行 v3.4.0：确定性 Task 1 scaffold/bootstrap → Gate 1 → Task 2 Evidence/strategy signal → Task 2.5 比较命题、选择 signature 成果 → 一次人工策略批准（`-auto` 为 assumed）→ safe-draft snapshot → 并行 Task 3 正向写作 → 批量独立 realization audit → realized-only 综述 → 装配 → 自适应红队/Gate 2 → 可选 audited PPT 结构稿 → report-anchored customer-fit → `validate-run` / `finalize-run`。图片生成、样张和 PPTX 装配由下游工作流完成。生成任务采用“目标—依据—工作顺序—安全替代—输出契约”，反模式只进入 critic 与复盘。

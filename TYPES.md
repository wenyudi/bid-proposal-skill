...
| `buyer_expert` | standard 合并 buyer + expert，去掉相邻重复根因 |
...
选择顺序：用户显式指定 → 命中信号最多 → 最高权重评分维度定夺 → custom。主叙事全案唯一，可有 secondary；报价、合规、资质始终用 logic/evidence 呈现。叙事只有 presentation authority，不裁剪 Requirement 或改变承诺。
...
---
`proposal skill · 3.1.0 · lean priors`

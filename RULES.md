...
- 每个 lead VP 至少有一个 required `visible_output`：只定义评委要检查的目的、支持对象、必填字段、grounding 和 truth boundary，不规定版式。writer 必须在正文自然填实；外部附件只进人工待办，不能替代正文成果。
...
---
```
proposal skill · 3.1.0 · lean policy
```

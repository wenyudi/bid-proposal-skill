...
- derived brief、coverage、diagnostics、fit、realization、PPT blueprint 与正文都不是事实源，不能反向静默改 canonical。
...
- Task 2 只能加 Evidence/Link、反证和 change proposal，不能静默改 Need 含义；会改变命题或价值机制的新证据以 `strategy_signal` 显式交给 Task 2.5。
...
- “想”和“写”分开：Task 1 保留候选 VP 与替代命题；Task 2.5 在研究后收敛一页纸策略，writer 不参与策略选择。人只在写作前批准一次核心张力、洞察、主张、推导、差异与落地逻辑。
- Task 2.5 可在既有 Need/VP/Claim/Action 与 Evidence 边界内更新 one-page strategy、Big Idea、win themes、narrative、Section strategy role 和选择状态。只有 Task 2 提供 evidence-backed `reopen_required + strategy_signal` 时，才可在既有 Role/Need/Criterion 上新建或改写 candidate VP，并保留 `research_signal_refs`；客户路径变化回 Task 1。Role/Need/Criterion/decision_paths/EvidenceLink 与 title/bid type/budget strategy/decision map 只读，不得 resolved/assumed Gate 或自批策略，不得新增/改变 authority、升 committed/confirmed，或扩大已授权语义。
- Big Idea 只是 approved core thesis 的表达投影，不是第二条真相；其文本必须与 recall line 完全相同，并与章节 spine 和 selected VP 同向。
...
## 7. 一页纸策略、DecisionJob、叙事与正文

- `strategy/v5` 的一页纸至少写清：客户张力、尖锐洞察及其非共识理由、核心命题与十秒记忆句、最终命题与最强替代的比较、决定性 refs、接受的取舍与改选信号、洞察→策略→表达→执行→证明、名称互换测试、proof plan、落地可信度和五维 ordinal 自评。政府/公共与商业模板分开；hybrid 必须声明主标准。
- 一页纸会进入 writer/redteam brief，只能保存可安全投影的综合判断与 refs；private 原句、个人偏好、关系、底价和未公开竞争情报仍只转成边界，不能藏进“洞察”。
- rubric 不计算总分。洞察锐度、可复述性、推导闭环、差异化和落地可信度逐项使用 deficient/fragile/adequate/strong/distinctive，并给具体 finding/next revision。自评不能替代独立 strategy critic 或人工批准。
- 名称互换测试中 `passes` 表示换成竞争对手仍成立，是失败信号；`fails` 才表示项目选择、真实机制或证据使其难替换。不能靠公司名、形容词或未经证实的资源优势制造失败。
- `approval=pending|changes_requested` 阻断写作；human `approved` 才允许直接递交。显式 `-auto` 或迁移只能写 `assumed`，可生成安全草案但阻断 submission。
...
- 每章还拥有一个 `strategy_role`：contribution / inherits / hands_off。编译器从这些字段生成唯一 section spine；每章必须推进核心命题，不得只重复记忆句或展示一组并列能力。
...
- “赋能、生态、闭环、矩阵、全域、多维”等不是机械禁词；删掉术语后若没有对象、机制、依据和可检查结果，删除整句重写。具体好坏对照优先于抽象禁令。
- 每个 lead VP 至少有一个 required `visible_output`：只定义评委要检查的目的、支持对象、必填字段、grounding 和 truth boundary，不规定版式。全案从已有成果中指定唯一 `display_role=signature` 并写入 `signature_output_ref`，用于承担核心主张的主亮点证明；其余成果按 supporting/reference 呈现。grounding 安全投影须含有填实必需内容，仅有风险边界摘要不足以冻结；writer 必须在正文自然填实，外部附件不能替代。
- 严格底层不进入客户稿：禁止 Role/Need/DecisionJob/Claim/Evidence ID、覆盖状态、customer-fit、状态机、RACI 机械标签和审计过程。用自然段、集中表格和顺滑过渡保护阅读体验。

### PPT 结构稿

- PPT blueprint 是已审计正文的派生压缩层，不是第六份 canonical。它绑定当前 generation snapshot 与 presentation brief；canonical、正文或 realization 变化后重新编译。
- 核心轨用最短页序完成客户张力→洞察→选择→signature→机制/行动→证明/保障→收束；附录轨承接 mandatory 对照、长名单、效果图系列、尺寸材质、预算和风险查验。页数服从必要判断与真实素材。
- 每张核心页推进一个客户判断，使用结论式标题、准确上屏文案、明确画面任务和承接/交出；全部 required presentation refs 在 core 或 appendix 得到覆盖。
- 全案只有一个 signature 页，必须引用已填实 `signature_output_ref`，并作为下游图片工作流的建议样张页。
- `generate/strict_input/style_reference` 分开；真实素材记录路径、用途、可用状态与 rights status。参考 PDF/PPT 提供可见风格，不取得方案事实权威。
- 图片生成、视觉风格、样张批准和 PPTX 装配属于下游工作流。结构校验通过只表示 `ready_for_outline_review`，不替代图片文字准确性、视觉 QA、真实性复核或 submission readiness。
...
- Task 只读 compile-context 生成的最小 brief；must_use 的 Requirement 原文、Claim 语义/scope/承诺、visible output、visibility 和硬边界不可裁剪。Task 2.5 还必须收到候选一页纸及其有权完整 upsert/校验的 Claim、Metric、交付、Gate 与 Section 对象。token 超限应拆任务或显式提高预算，不能静默删约束。
- 每个 section brief 始终携带批准后的一页纸安全投影、全案 section spine、本章 strategy role 和前序 canonical 骨架摘要；不依赖尚未生成的前文正文，因此仍可并行写作。
...
结构根因分类：uncovered、orphan、unsupported、unowned、unmeasured、overcommitted、contradictory、redundant。策略质量另只选一个主失败：strategy_hollow（策略空洞）、throughline_break（主线断裂）、cliche_style（文风套话）。先修最上游根因，再看下游问题是否消失；同一根因聚合 affected paths，修复回最近 owner。
...
- filled visible output 只证明成果完整，不证明阅读效率。`reading_efficiency` 只能来自当前 snapshot、能在当前报告中定位 exact quote 的独立语义判断；同一条件也约束由红队提升的 insight/differentiation judgment。
...
---
```
proposal skill · 3.4.0 · presentation-ready policy
```

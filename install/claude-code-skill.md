...
2. 创建入口目录：`mkdir -p ~/.claude/skills/proposal`
3. 把下面的模板写入 `~/.claude/skills/proposal/SKILL.md`，**把 `{REPO}` 全部替换成第 1 步的绝对路径**
4. 新开一个 Claude Code 会话，输入 `/proposal <标书路径>` 验证
...
---
name: proposal
description: 政企传媒投标方案生成 — 读甲方标书拆评分项保基础得分，按标选叙事策略（逻辑征服/故事打动/愿景共创/数据实证），联网调研甲方/行业/案例 + 从本地案例库自动筛选案例，并行分章撰写，应标对照表零遗漏校验。Use when 用户要写投标方案/应标文件/给政企客户的提案，或提供标书要求出方案，或输入 /proposal。
---
...
2. **严格按其"主 agent 调度流程"执行**：Setup → 语言判定 → 标书摄入+模式/叙事解析 → Task1 标书解读 → Task2 联网情报（含案例库筛选）→ Task3 并行分章 → Task4 装配+合规校验+自评+QA。
...
- **合规阻断**：`prop_tools.py check-compliance` 有 missing_mandatory 必须补齐才能交付，这是唯一硬阻断点。
````

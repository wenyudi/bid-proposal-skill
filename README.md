...
- 把"想"和"写"分开：先判**制胜轴**（这标靠管理可靠赢还是创意打动赢），再发散多个主张、按短板 + 差异化收敛一版**制胜一页纸**（含最强替代与互换测试），一次人工确认后再并行分章写作。
- 缺失素材用**具体、可信**的内容虚构补全，让方案读起来成熟完整；每处虚构源头申报进 `_风险与待核实.md`，按 权重×风险 排序供你核实替换。
- 独立于 writer 的**三-lens 复核**（覆盖 / 对手视角抓"合规但空" / **客户体验**抓内部泄漏与罗列），循环到过。
- 默认**视觉分镜式**图片 PPT 结构稿（页多、每页一个画面、子部件各拆页；**完整商业稿**：缺真图用意向图顶位、内部字段绝不上屏、outline 带素材替换清单），交给图片工作流（image2 / kv-studio）。

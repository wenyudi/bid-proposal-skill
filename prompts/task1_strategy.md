...
6. **写一页纸与逐章骨架**（见输出契约）：给每章分配认领的评分项与**深度取向**（高权重/ceiling 章写最深）——**不设字数上限，独有信息量多比少好，把该章评分项和价值写透即可、不注水；low 深度章的体量不应逼近 high 章（篇幅分布本身就是给评审的权重信号）**；指定全案唯一 signature 主亮点（**落在制胜轴上：管理制胜=关键成果/机制图，创意制胜=核心创意 KV/大 idea**）；**先按制胜轴、再按标型**从 `narratives.json` 的 key（logic/story/vision/evidence）选叙事（创意制胜别默认 logic）并写理由。骨架里章标题句式要多样（不全用"主张：副题"腔），并给每章标**体裁基调**（论证型/表格型/朴素说明型——报价、资质、售后类章朴素准确即可，不逼每章立判断）。
7. **素材分配表**（防跨章复读的源头机制）：把核心案例、外部先例、关键数据、方法框架、合规硬指标逐条列出，各指定**唯一宿主章**（在宿主章完整展开一次，其他章只许一句带过）；合规硬指标（人数/规格/频次/资质等）指定一个"底盘表"宿主章集中逐项应答，其余章只写差异项。
8. **内行颗粒清单**：列出本标型内行才写得出的 5–10 个专业细节（平台投放工具与预算杠杆、达人刊例口径、审批真实归口、行业规范/色号/工艺参数、关键岗位角色如银行客户经理），逐条标注素材里有没有；没有的标"需虚构+登记"，交 writer 落实。**缺这一层，方案再工整也会被内行看出没做过。**
...
- 逐章骨架表：章号 | 标题（句式多样，不全用主张腔）| 独有贡献 | 认领评分项 id | 深度取向(high/med/low) | 体裁基调(论证/表格/朴素) | 承接/交出
- **素材分配表**：素材/论据/案例/合规硬指标 | 唯一宿主章 | 其他章允许的提及方式（一句带过/不提）
- **内行颗粒清单**：专业细节 | 素材有无 | 需虚构+登记的标注
- 被否主张角度（各一句理由）
...
- 素材分配表覆盖全部核心论据与合规硬指标，每条有唯一宿主章；内行颗粒清单非空且逐条标注了素材有无。创意制胜标：多个创意交付物不共用同一模板（同构创意=一个创意的 N 次换皮）。

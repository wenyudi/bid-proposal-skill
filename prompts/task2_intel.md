...
6. 留意会改变初始策略空间的新证据：它若改变核心命题，记录为 `affects=thesis`；若形成现有候选未覆盖的新价值机制，记录为 `affects=value_proposition`。
...
```json
{
  ...
  "strategy_signals": [
    {"id": "SIG-*", "observation": "研究确认的新事实或因果", "evidence_refs": ["EV-*"], "target_refs": ["NEED/CRIT/VP/CL-*"], "affects": "thesis|value_proposition", "strategic_implication": "它怎样改变原候选的选择", "confidence": "high|medium|low"}
  ],
...
`strategy_signals` 只承载有 Evidence ref、会实质改变选择的新发现；常规补强继续进入 Evidence/Link。出现新的价值机制时把 `reopen_required` 写为 true，让 Task 2.5 在既有 Role/Need/Criterion 上重新打开候选；客户路径本身需要变化时，同时写 gap 并交回 Task 1。
...
- 策略信号逐条连接 Evidence，并说明它改变的是命题还是价值机制；`reopen_required` 与信号一致。
...
Evidence: <数> · Links: <数> · Gaps: <数> · Signals: <数> · Reopen: <yes|no> · Sources: <数>
```

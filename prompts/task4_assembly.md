...
writer 不生成 hints。Requirement addressed、Claim/Action entailed、required visible output filled 才是 valid；invalid 按 owner 回上游，不改 manifest 状态。
...
每次从返回值重新绑定 `$REPORT/$BUNDLE/$BRIEF`。`$AUTO_ARG` 只有 `-auto` 时为 `--auto`；它不代表可递交。
...
`finalize-run` 复用一次 canonical submission check，聚合 compliance、QA、ordinal fit、todo、Gate 2 和 archive，并签发 state/report-bound receipt。草案加 `--allow-draft`，receipt 必须显示 `draft_only`；fatal/schema/source 损坏仍拒绝。归档成功后才清 TMPDIR，last-good 不得丢失。

## 验收

- Requirement 映射与正文响应零遗漏；所有 section/summary realization valid。
- lead 的 required visible output 已填实，未以未来附件或通用流程替代。
- Claim/Evidence/Metric/authority 与 Action/Owner/Resource/Acceptance 均在真实边界内。
- receipt 为 `submission_ready`，或首句明确“草案，不可直接递交”。
- `_内部研判.md`、`_人工待办.md`、`_acceptance-receipt.json`、`_state/` 均不递交，last-good 可恢复。
